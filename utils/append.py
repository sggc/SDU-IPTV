...
def merge_m3u(input_files, search_group_title="山东频道"):
    ...
    with open(input_files[0], "w") as output:
        output.writelines(lines[:insert_position])
        output.writelines(second_file_lines)
        output.write("\n")
        output.writelines(lines[insert_position:])

    print(f"合并完成，输出到 {input_files[0]}")


m3u_files = glob.glob("append/multicast/*.m3u")
